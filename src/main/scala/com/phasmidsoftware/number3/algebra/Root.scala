...
/**
  ...
  */
case class Root(n: Int, base: Number) extends Monotone with MultiplicativeWithPower[Root] with Ordered[Root] {
  def compare(that: Root): Int = (this.isExact, that.isExact) match {
    case (true, true) =>
  ...
  def compareExact(that: Monotone): Option[Int] = that match {
    case Root(m, x) if m == n => // TODO - there are other situations where the result should be Some(0) (see Factor class).
      Some(base.compare(x))
    case _ =>
      None
  }
  ...
  /**
    ...
    */
  def signum: Int = compare(Root.zero)
  ...
  /**
    ...
    */
  def toRational: Option[Rational] = base.toRational
  ...
  /**
    ...
    */
  def render: String = {
    val prefix = base.render
    val suffix = "√" // TODO this isn't quite right.
...
object Root {
  ...
  /**
    ...
    */
  val zero: Root = Root(1, Real.zero)
  ...
  /**
    ...
    */
  val one: Root = Root(1, WholeNumber.one)
  val nan: Root = Root(0, RationalNumber(Rational.NaN))
  ...
  /**
    ...
    */
  implicit val showRoot: Show[Root] = Show.show(_.render)
  ...
  /**
    ...
    */
  implicit object rootIsCommutativeGroup extends CommutativeGroup[Root] {
    /**
