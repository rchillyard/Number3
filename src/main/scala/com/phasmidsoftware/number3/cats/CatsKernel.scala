...
package com.phasmidsoftware.number3.cats

import cats.Show
import cats.implicits.catsSyntaxEq
import cats.kernel.{Eq, Order, PartialOrder}
import com.phasmidsoftware.number3.algebra._
import com.phasmidsoftware.number.core.algebraic.Algebraic
import com.phasmidsoftware.number.core.inner.{Factor, Rational, Value}
import com.phasmidsoftware.number.core.{Complex, ComplexCartesian, ComplexPolar, ExactNumber, Field, GeneralNumber, Real}
...
trait CatsKernelInstances {
  ...
  // ExactNumber
  implicit val exactNumberEq: Eq[Number] = Eq.instance(_ == _)
  implicit val exactNumberOrder: Order[Number] = Order.from((x, y) => x.compare(x, y))
  implicit val exactNumberShow: Show[Number] = Show.show(_.render)
