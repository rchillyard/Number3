...
import com.phasmidsoftware.number3.mill.Mill
...
class ShuntingYardParserSpec extends AnyFlatSpec with should.Matchers {
  ...
  it should "parseInfix 1" in {
    ...
    val xo: Option[Valuable] = for (s <- sy.toOption; e <- s.evaluate; n = e.materialize) yield n
    xo should matchPattern { case Some(_) => }
    xo.get shouldBe Valuable.one
  }
  ...
  it should "parseInfix ( ( ( 4 + 5 ) * ( 2 + 3 ) + 6 ) / ( 8 + 7 ) ) ∧ 9" in {
    ...
    val xo: Option[Valuable] = for (s <- sy.toOption; e <- s.evaluate; n = e.materialize) yield n
    xo should matchPattern { case Some(_) => }
    xo.get shouldBe Real("60716.992766464000")
  }
  ...
  it should "parseInfix 3 + 4 × 2 / ( 1 − 5 ) ∧ 2 ∧ 3" in {
    ...
    val xo: Option[Valuable] = for (s <- sy.toOption; e <- s.evaluate; n = e.materialize) yield n
    xo should matchPattern { case Some(_) => }
    xo.get shouldEqual Real(3.0001220703125)
  }
  it should "parseInfix ( 1 + ( ( 2 + 3 ) * ( 4 * 5 ) ) )" in {
    ...
    val xo: Option[Valuable] = for (s <- sy.toOption; e <- s.evaluate; n = e.materialize) yield n
    xo should matchPattern { case Some(_) => }
    xo.get shouldBe Valuable(101)
  }
  it should "parse Infix and evaluate:  9" in {
    val value: Try[Mill] = p.parseInfix("3 ∧ 2")
    value should matchPattern { case Success(_) => }
    val evaluationResult = value.get.evaluate
    evaluationResult.isDefined shouldBe true
    evaluationResult.get.materialize shouldBe Some(Valuable(9))
  }
  it should "parse Infix and evaluate:  0.5" in {
    ...
    val evaluationResult = value.get.evaluate
    evaluationResult.isDefined shouldBe true
    evaluationResult.get.materialize shouldBe Some(Valuable.half)
  }
